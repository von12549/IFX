...
| Gate | 性质 | 阻塞范围 |
| --- | --- | --- |
| Gate 1：事务边界 | 硬依赖 | Outbox/Inbox 原子性、事件最终验收 |
| Gate 2：数据库边界 | 硬依赖 | Outbox/Inbox 表归属、migration 与关系数据库测试 |
| Gate 3：Contract/Event 治理 | 设计前置 | Contracts surface、event ownership 与版本策略 |
...
- [ ] GOV-G2 保存批准后的能力/事件目录，作为 Contracts 分类和 LayerGuard allowlist 的输入证据。
