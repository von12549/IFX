...
- `V3_backup` remains unchanged as the portable-package reference. At migration time it contained 25 files. Eight generic V3 files had subsequently changed in `V3`; their current versions were brought into `V3_ifx` so the IFX fork includes the non-vacuous detector and input-safety fixes. No change was written to `V3_backup` or the old gate.
