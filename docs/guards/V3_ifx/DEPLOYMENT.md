...
To activate this as a merge gate, add a CI job that runs both `Check` commands, both `Test` commands, and `Diff` with the PR base/head commits in a clean checkout, then mark the job required in repository protection. Until that setup is verified, a local pass is evidence only; it does not block merges. Keep the existing specialized jobs active during this parallel migration.
