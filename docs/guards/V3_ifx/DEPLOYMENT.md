...
PR #26 run `34978867655` passed every V3 job on commit `073ebe4`. The current private GitHub Free repository cannot configure branch protection, rulesets or required checks, so merge enforcement is manual: confirm the latest `v3-*` jobs succeeded before merge. The seven legacy workflows retain only `workflow_dispatch`; run them manually for rollback comparison. Do not delete those workflows, root validators, `src/layerguard.json`, or non-V3 guard documentation until a future Pro/Team upgrade enables required-check enforcement and the remaining Plan 05 deletion gates. `mcp/LayerGuard` and its historical baselines remain intact throughout.
