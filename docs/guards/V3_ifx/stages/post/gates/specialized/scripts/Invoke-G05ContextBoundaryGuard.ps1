...
if ([string]::IsNullOrWhiteSpace($ReportPath)) {
    $ReportPath = "artifacts/guards/v3-ifx/specialized/g05/G05-phase$Phase-guard-report.json"
}
$resolvedReportPath = if ([System.IO.Path]::IsPathRooted($ReportPath)) { $ReportPath } else { Join-Path $repositoryRoot $ReportPath }
$inventoryPath = Join-Path (Split-Path -Parent $resolvedReportPath) 'G05-context-inventory.json'
...
$checks = [ordered]@{
    ...
    durableMessagingCapabilitiesRecordedTruthfully = $inventory.messagingCapabilities.durableOutboxImplemented -and $inventory.messagingCapabilities.durableInboxImplemented -and -not $inventory.messagingCapabilities.deadLetterOrQuarantineImplemented -and -not $inventory.messagingCapabilities.replayOrReprocessingImplemented
...
if ($Phase -ge 9) {
    $verificationPath = Join-Path $PSScriptRoot 'Invoke-G05Verification.ps1'
    $workflowPath = Join-Path $repositoryRoot '.github/workflows/v3-ifx-guardrails.yml'
    $baselinePath = Join-Path $repositoryRoot 'docs/architecture/review/gates/G05/verification-baseline-v1.json'
    $catalogValidator = Get-Content -Raw -LiteralPath (Join-Path $PSScriptRoot 'Test-G03ContractEventCatalog.ps1')
    ...
    $checks.singleLocalCiVerificationEntryPointExists = (Test-Path $verificationPath) -and $workflowSource -match 'SpecializedGate G05' -and $workflowSource -match 'upload-artifact'
    $checks.verificationRunsOnlyOwnedChecks = $verificationSource -match 'Invoke-G05ContextBoundaryGuard\.ps1.*-Phase 11' -and $verificationSource -match 'Test-Plan05SecurityBoundary\.ps1' -and $verificationSource -notmatch 'Invoke-(LayerGuard|IFX)\.ps1'
    $checks.verificationDelegatesSolutionToQuality = $verificationSource -notmatch 'dotnet (restore|build|test)' -and $workflowSource -match 'v3-quality-solution'
    ...
    $checks.verificationBaselineIsTruthfulAndBounded = $verificationBaseline.status -eq 'plan05-repository-verification' -and $verificationBaseline.minimumSolutionTests -ge 1041 -and $verificationBaseline.requiredResults.layerGuardNewViolations -eq 0 -and $verificationBaseline.truthfulBoundary -match 'Plan 05' -and $verificationBaseline.truthfulBoundary -match 'target data, live IdP, production telemetry and named approvals remain external'
    $checks.catalogMutationBaselineIncludesCredentialBoundaries = $verificationBaseline.requiredResults.catalogMutationSelfTests -eq 18 -and @($verificationBaseline.requiredCatalogMutationNames | Select-Object -Unique).Count -eq 18 -and @('durable credential protocol', 'logged credential protocol', 'unknown credential consumer' | Where-Object { $_ -notin $verificationBaseline.requiredCatalogMutationNames }).Count -eq 0
