...
$generatedManifestPath = Join-Path (Split-Path -Parent $resolved) 'release-runtime-manifest.generated.json'
& (Join-Path $PSScriptRoot 'New-G04ReleaseManifest.ps1') -OutputPath $generatedManifestPath
$first = Sha $generatedManifestPath
& (Join-Path $PSScriptRoot 'New-G04ReleaseManifest.ps1') -OutputPath $generatedManifestPath
$second = Sha $generatedManifestPath
...
$release = Get-Content -Raw -LiteralPath $generatedManifestPath | ConvertFrom-Json -Depth 100
