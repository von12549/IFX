...
Every policy file here except the derived projections is registered in `../shared/policy-config.json` (Plan 06 D24). A semantic change of a registered file needs a base `weaken-policy` authorization that the change pull request consumes. A projection must match what the base generator produces from its authority source.
