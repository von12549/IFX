...
if ($publicEntries -contains 'docs/guards/V3_ifx/commands/Invoke-IFXGuardrails.ps1') {
    $publicEntries += @(
        ...
        'docs/guards/V3_ifx/trusted-base/Test-IFXTrustedBaseCandidate.ps1'
    )
}
...
if ($null -ne $changeScope) {
    ...
    $scopeWorkflowEntryPoint = if ($scopeEntryPoint -ceq 'docs/guards/V3_ifx/commands/Invoke-IFXGuardrails.ps1' -and $workflowText.Contains('$env:GUARD_BASE/docs/guards/V3_ifx/trusted-base/Invoke-IFXTrustedBase.ps1', [StringComparison]::Ordinal)) {
        'docs/guards/V3_ifx/trusted-base/Invoke-IFXTrustedBase.ps1'
    ...
    foreach ($jobId in @($changeScope.candidateStepJobs)) {
        ...
        $classifies = $step -ne '' -and
            ...
            ($scopeWorkflowEntryPoint -cne $scopeEntryPoint -or $step -match '-TrustedBase\b') -and
