...
The [migration record](architecture/IFX-MIGRATION.md) identifies the ownership and cutover boundary. `.github/workflows/v3-ifx-guardrails.yml` is the sole PR/main guard trigger. Private-repository branch protection is unavailable on the current GitHub Free plan, so its successful jobs are reviewed manually before merge. Legacy workflow files are `workflow_dispatch`-only and cannot be removed until a future Pro/Team required-check cutover satisfies Plan 05's deletion threshold.
