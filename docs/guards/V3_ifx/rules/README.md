...
This directory is a **guide**, not a second active rule source. `Invoke-V3.ps1` resolves `stages/post/rules/*.json` through `shared/profile-layout.json` for Plan/Pre association and the narrow V3 stage detector. `Invoke-IFX.ps1` reads `policy/layerguard.json` for the independent, full LayerGuard architecture gate. The nine numbered IDs appear in both systems so Agents can name the same concern, but their coverage differs. `examples/minimal/rules/` is synthetic.
