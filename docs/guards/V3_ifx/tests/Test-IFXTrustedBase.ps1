...
try {
    ...
    if ($DiffConsumptionOnly) {
        ...
        $candidateRegistryHead = New-PlannedHead 'candidate-registers-new-layout' $authorizedBase {
            ...
            Edit-Json 'docs/guards/V3_ifx/shared/policy-config.json' { param($d) @($d.entries | Where-Object { $_.id -eq 'layerguard-policy' })[0].paths += 'docs/guards/V3_ifx/policy/fixture-extra.json' }
        }
