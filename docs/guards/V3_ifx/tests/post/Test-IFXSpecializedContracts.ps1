...
$legacyComplete = @($requiredSpecializedFiles | Where-Object { -not [IO.File]::Exists((Join-Path $legacySpecialized $_)) }).Count -eq 0
$stageComplete = @($requiredSpecializedFiles | Where-Object { -not [IO.File]::Exists((Join-Path $stageSpecialized $_)) }).Count -eq 0
if ($legacyComplete -eq $stageComplete) { throw 'Specialized gates must have exactly one complete legacy or stage-owned layout.' }
$specializedRoot = if ($stageComplete) { $stageSpecialized } else { $legacySpecialized }
$fixture = Join-Path $root "artifacts/guards/v3-ifx/specialized-contract-$([Guid]::NewGuid().ToString('N'))"
New-Item -ItemType Directory -Force -Path $fixture | Out-Null
try {
    $resultSchema = Join-Path $specializedRoot 'contracts/detector-result.schema.json'
    $fixtureSchema = Join-Path $specializedRoot 'contracts/fixture.schema.json'
    ...
    $output = @(& pwsh -NoProfile -File (Join-Path $specializedRoot 'scripts/Test-Plan04ExtractionPolicy.ps1') -PolicyPath $missingPolicy -StatusPath $missingReport 2>&1)
