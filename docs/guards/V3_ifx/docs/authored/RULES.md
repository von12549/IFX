...
This directory is a **guide**, not a second active rule source. `Invoke-V3.ps1` resolves `stages/post/rules/*.json` through `shared/profile-layout.json` for Plan/Pre association and the narrow V3 stage detector. `Invoke-IFX.ps1` reads `stages/post/policy/layerguard.json` for the independent, full LayerGuard architecture gate. The nine numbered IDs appear in both systems so Agents can name the same concern, but their coverage differs. `examples/minimal/rules/` is synthetic.
...
```json
{
  ...
  "coverage": "none",
  "authority": "stages/post/policy/layerguard.json:reviewed-location",
...
1. Read the relevant `stages/post/rules/*.json`, `stages/post/policy/layerguard.json` entry, current source paths, reviewed [IFX architecture evidence](../../stages/analysis/evidence/ARCHITECTURE.md), and the current runtime review under `artifacts/guards/v3-ifx/analysis/`. State which gate proves each part of the concern.
2. Keep numbered rule IDs aligned with `stages/post/policy/layerguard.json:ruleRefs`; run `Invoke-IFX.ps1 -Mode Validate`. The alignment check verifies IDs and stage authority markers, not full semantic equivalence.
