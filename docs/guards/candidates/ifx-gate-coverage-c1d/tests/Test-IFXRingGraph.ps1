[CmdletBinding()]
param(
    [string] $EvidenceRoot = 'artifacts/guards/p10-ifx-c1d/test-runs',
...
$moduleRoot = Join-Path $candidateRoot 'modules/ifx-ring-graph'
...
Assert ($manifest.id -ceq 'ifx-ring-graph' -and @($manifest.stages) -join ',' -ceq 'pre') 'Module identity/Stage drift.'
...
Assert ((Hash (Join-Path $repoRoot $policy.sourceRule.path)) -ceq $policy.sourceRule.sha256) 'V3 L2.2 source drift.'
foreach ($field in @('rings', 'allowedDependencies', 'transitiveBoundaryRoles')) {
    Assert (($policy.$field | ConvertTo-Json -Depth 30 -Compress) -ceq
        ($source.$field | ConvertTo-Json -Depth 30 -Compress)) "Frozen $field projection differs from V3 policy."
}
...
Assert ($fixtures.formatVersion -eq 1 -and @($fixtures.cases).Count -eq 13) 'Fixture identity drift.'
foreach ($case in $fixtures.cases) {
    ...
    if ($case.id -ceq 'clean') {
        $poison = Join-Path $target 'guard/IFX.Modules.CRM.Domain/IFX.Modules.CRM.Domain.csproj'
    ...
    Assert (@($result.findings).Count -eq $case.expectedFindings) "Finding count mismatch: $($case.id)."
    foreach ($finding in $result.findings) {
        Assert ($finding.ruleId -ceq 'RING-DIRECTION' -and $finding.detectorId -ceq 'ifx-ring-graph' -and
...
$drifted = Invoke-Adapter (Join-Path $runRoot 'clean') ([ordered]@{ enabledClaims = @($policy.claimId); policySha256 = ('0' * 64) })
...
Assert ($realBefore -ceq $realAfter -and $realResult.status -in @('pass', 'fail') -and
    $realResult.coverage[0].matched -gt 0) 'Real IFX graph candidate could not be evaluated safely.'

# The optional published-base synthetic Host check uses only a test-fixture review.
...
if ($providedBasePaths.Count -eq 3) {
    ...
    $bundleModuleRoot = Join-Path $bundlePackage 'modules/ifx-ring-graph'
    ...
    $profileId = 'ifx_c1d_fixture'
    $profilePath = Join-Path $bundlePackage "profiles/catalog/$profileId/profile.json"
    $profile = [ordered]@{
        formatVersion = 1
        id = $profileId
        version = '0.1.0'
        projectIdentity = [ordered]@{ id = 'ifx-c1d-fixture'; relativeRoots = @('src') }
        moduleSelections = @([ordered]@{ id = 'ifx-ring-graph'; versionRange = '>=0.1.0 <1.0.0'; config = $config })
        stageConfiguration = [ordered]@{
            bootstrap = [ordered]@{ enabled = $false; modules = @() }
            analysis = [ordered]@{ enabled = $false; modules = @() }
            pre = [ordered]@{ enabled = $true; modules = @('ifx-ring-graph') }
    ...
    $bundleManifest = [ordered]@{
        formatVersion = 1
        id = 'ifx-c1d-synthetic-extension'
        version = '0.1.0'
        compatibleApi = '1.x'
        baseVersion = '1.1.2'
        profiles = @([ordered]@{ id = $profileId; version = '0.1.0'; path = "profiles/catalog/$profileId/profile.json"; sha256 = Hash $profilePath })
        modules = @([ordered]@{ id = 'ifx-ring-graph'; version = '0.1.0'; manifestPath = 'modules/ifx-ring-graph/module.json'; manifestSha256 = Hash (Join-Path $bundleModuleRoot 'module.json'); allowedCapabilities = $capabilities })
        files = $bundleFiles
    }
    ...
    $review = [ordered]@{
        formatVersion = 1
        id = '20260923-ifx-c1d-synthetic-fixture'
        scope = 'synthetic-test-only'
        decision = 'accepted'
        acceptedBy = [ordered]@{ authorityType = 'test-fixture'; authorityId = 'ifx-c1d-synthetic-fixture'; candidateHostVerdictAllowed = $false }
        bundleManifestSha256 = Hash $bundleManifestPath
        baseArchiveSha256 = $baseReceipt.archiveSha256
        moduleCeilings = @([ordered]@{ moduleId = 'ifx-ring-graph'; allowedCapabilities = $capabilities })
    }
    ...
    foreach ($case in @(
        [ordered]@{ id = 'clean'; status = 'pass'; category = 'success'; matched = 6 },
        [ordered]@{ id = 'direct'; status = 'fail'; category = 'findings-blocking'; matched = 6 },
        [ordered]@{ id = 'transitive'; status = 'fail'; category = 'findings-blocking'; matched = 6 },
        [ordered]@{ id = 'zero-domain'; status = 'fail'; category = 'findings-blocking'; matched = 5 }
    )) {
        $target = Join-Path $runRoot $case.id
...
Write-Output "IFX C1d graph candidate passed thirteen fixtures and real IFX scan ($($realResult.status), $(@($realResult.findings).Count) findings). Evidence root: $runRoot"
