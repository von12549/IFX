[CmdletBinding()]
param(
    [string] $EvidenceRoot = 'artifacts/guards/p10-ifx-c1c/test-runs',
...
$moduleRoot = Join-Path $candidateRoot 'modules/ifx-package-reference'
...
Assert ((Hash (Join-Path $repoRoot $policy.sourcePolicy.path)) -ceq $policy.sourcePolicy.sha256) 'V3 policy source drift.'
Assert ((Hash (Join-Path $repoRoot $policy.sourceRule.path)) -ceq $policy.sourceRule.sha256) 'V3 L3.4 source drift.'
foreach ($field in @('rings', 'allowedPackages', 'forbiddenPackages')) {
    $projected = $policy.$field | ConvertTo-Json -Depth 20 -Compress
    $authority = if ($field -eq 'rings') {
        $subset = [ordered]@{}
        foreach ($ring in @('Domain', 'Contracts', 'Application', 'Presentation')) { $subset[$ring] = $source.rings.$ring }
        $subset | ConvertTo-Json -Depth 20 -Compress
    } else { $source.$field | ConvertTo-Json -Depth 20 -Compress }
    Assert ($projected -ceq $authority) "Frozen $field projection differs from V3 policy."
}
Assert ($rulePlan.moduleId -ceq $manifest.id -and $rulePlan.matrixSha256 -ceq (Hash $policyPath) -and
    @($rulePlan.rules).Count -eq 2 -and
    ($rulePlan.rules.ruleId -join ',') -ceq 'RING-PACKAGE,RING-PACKAGE-FORBIDDEN') 'Rule execution plan drift.'

$config = [ordered]@{ enabledClaims = @($policy.claimIds); policySha256 = Hash $policyPath }
...
Assert ($fixtures.formatVersion -eq 1 -and @($fixtures.baseProjects).Count -eq 4) 'Fixture identity drift.'
foreach ($case in $fixtures.cases) {
    $target = Join-Path $runRoot $case.id
    [void][IO.Directory]::CreateDirectory($target)
    foreach ($project in $fixtures.baseProjects) {
        if (@($case.omitRings) -contains $project.ring) { continue }
        ...
        [void][IO.Directory]::CreateDirectory((Split-Path -Parent $path))
        $replacement = @($case.overrides | Where-Object ring -eq $project.ring)
        $content = if ($replacement.Count -eq 1) { [string]$replacement[0].content } else { [string]$project.content }
        [IO.File]::WriteAllText($path, $content, [Text.UTF8Encoding]::new($false))
    }
    if ($case.PSObject.Properties.Name -contains 'linkSourceFrom') {
        $from = Join-Path $runRoot "$($case.linkSourceFrom)/src"
        $link = Join-Path $target 'src'
        $kind = if ($IsWindows) { 'Junction' } else { 'SymbolicLink' }
        New-Item -ItemType $kind -Path $link -Target $from | Out-Null
    }
    $before = Inventory $target
    $result = Invoke-Adapter $target $config
    $after = Inventory $target
    Assert ($before -ceq $after) "TargetRoot bytes changed: $($case.id)."
    Assert ($result.status -ceq $case.expectedStatus -and $result.exitCategory -ceq $case.expectedExitCategory) "Status/category mismatch: $($case.id): $($result | ConvertTo-Json -Depth 8 -Compress)"
    Assert ((@($result.findings | ForEach-Object ruleId) -join ',') -ceq ($case.expectedRules -join ',')) "Finding rules mismatch: $($case.id)."
    Assert (($result.coverage.matched -join ',') -ceq ($case.expectedCoverage -join ',')) "Coverage mismatch: $($case.id)."
    Assert (($result.coverage.claimId -join ',') -ceq ($policy.claimIds -join ',')) "Claim identity mismatch: $($case.id)."
    foreach ($finding in $result.findings) {
        Assert ($finding.detectorId -ceq 'ifx-package-reference' -and $finding.severity -ceq 'blocking') "Finding identity mismatch: $($case.id)."
    }
...
$drifted = Invoke-Adapter $cleanTarget ([ordered]@{ enabledClaims = @($policy.claimIds); policySha256 = ('0' * 64) })
...
Assert ($realResult.status -ceq 'pass' -and $realResult.exitCategory -ceq 'success' -and
    $realResult.coverage[0].matched -ge 1 -and $realResult.coverage[1].matched -ge 1) 'Real IFX package-reference candidate did not pass.'
...
if ($providedBasePaths.Count -eq 3) {
    ...
    $bundleModuleRoot = Join-Path $bundlePackage 'modules/ifx-package-reference'
    ...
    $profileId = 'ifx_c1c_fixture'
    $profilePath = Join-Path $bundlePackage "profiles/catalog/$profileId/profile.json"
    $profile = [ordered]@{
        formatVersion = 1
        id = $profileId
        version = '0.1.0'
        projectIdentity = [ordered]@{ id = 'ifx-c1c-fixture'; relativeRoots = @('src') }
        moduleSelections = @([ordered]@{ id = 'ifx-package-reference'; versionRange = '>=0.1.0 <1.0.0'; config = $config })
        stageConfiguration = [ordered]@{
            bootstrap = [ordered]@{ enabled = $false; modules = @() }
            analysis = [ordered]@{ enabled = $false; modules = @() }
            pre = [ordered]@{ enabled = $true; modules = @('ifx-package-reference') }
            post = [ordered]@{ enabled = $false; modules = @() }
        }
        rules = @('RING-PACKAGE', 'RING-PACKAGE-FORBIDDEN')
        baselineRefs = @()
    }
    ...
    $bundleManifest = [ordered]@{
        formatVersion = 1
        id = 'ifx-c1c-synthetic-extension'
        version = '0.1.0'
        compatibleApi = '1.x'
        baseVersion = '1.1.2'
        profiles = @([ordered]@{ id = $profileId; version = '0.1.0'; path = "profiles/catalog/$profileId/profile.json"; sha256 = Hash $profilePath })
        modules = @([ordered]@{ id = 'ifx-package-reference'; version = '0.1.0'; manifestPath = 'modules/ifx-package-reference/module.json'; manifestSha256 = Hash (Join-Path $bundleModuleRoot 'module.json'); allowedCapabilities = $capabilities })
        files = $bundleFiles
    }
    ...
    $review = [ordered]@{
        formatVersion = 1
        id = '20260923-ifx-c1c-synthetic-fixture'
        scope = 'synthetic-test-only'
        decision = 'accepted'
        acceptedBy = [ordered]@{ authorityType = 'test-fixture'; authorityId = 'ifx-c1c-synthetic-fixture'; candidateHostVerdictAllowed = $false }
        bundleManifestSha256 = Hash $bundleManifestPath
        baseArchiveSha256 = $baseReceipt.archiveSha256
        moduleCeilings = @([ordered]@{ moduleId = 'ifx-package-reference'; allowedCapabilities = $capabilities })
    }
    ...
    foreach ($case in @(
        [ordered]@{ id = 'clean'; status = 'pass'; category = 'success'; coverage = '3,2' },
        [ordered]@{ id = 'explicit-deny'; status = 'fail'; category = 'findings-blocking'; coverage = '3,2' },
        [ordered]@{ id = 'zero-domain'; status = 'fail'; category = 'findings-blocking'; coverage = '2,2' }
    )) {
        $target = Join-Path $runRoot $case.id
        ...
        Assert (($hostResult.coverage.matched -join ',') -ceq $case.coverage) "Synthetic Host coverage mismatch: $($case.id)"
...
Write-Output "IFX C1c package-reference candidate passed eight fixtures, source projection, hashes, schema and real IFX scan. Evidence root: $runRoot"
