...
if ($hostAssemblyVersion -cne '1.1.3.0' -or $companionAssemblyVersion -cne '1.1.3.0') {
    $failures.Add("1.1.3 assembly versions are invalid: Host=$hostAssemblyVersion Companion=$companionAssemblyVersion")
}
...
if ($a.Code -eq 0 -and $b.Code -eq 0) {
    ...
    if ($resultA.version -cne '1.1.3' -or $resultB.version -cne '1.1.3') { $failures.Add('distribution product version is not 1.1.3') }
