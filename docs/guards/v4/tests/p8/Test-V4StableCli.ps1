...
try {
    ...
    Expect $version 0 'version' '"version"\s*:\s*"1\.1\.2"'
    ...
    foreach ($entry in Get-ChildItem (Join-Path $packageRoot 'modules') -Filter module.json -Recurse) {
        ...
        $expectedVersion = if ($module.id -ceq 'architecture-conformance') { '1.0.1' } else { '1.0.0' }
        if ($module.version -cne $expectedVersion) { $failures.Add("$($module.id) component version is not $expectedVersion in V4 1.1.2.") }
    ...
    Write-Host 'V4 stable CLI tests passed: product version 1.1.2, API-compatible module versions, contract validation behavior, required-root syntax and Linux/Windows-only declarations.'
}
finally {
