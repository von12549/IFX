...
$fixtures = [ordered]@{
    ...
    state = [ordered]@{ formatVersion = 1; projectInstances = @([ordered]@{ id = ('d' * 32); targetCanonicalPath = 'C:/target'; targetIdentityHash = $hash; profileId = 'synthetic_profile' }); transactions = @([ordered]@{ id = ('e' * 32); kind = 'state-write'; status = 'applied'; manifestHash = $hash }); resetReceipts = @([ordered]@{ id = ('f' * 32); mode = 'project'; acceptedManifestHash = $hash; beforeHash = $hash; afterHash = $hash }) }
...
$schemaNames = @('capability-matrix','cli-contract','genesis-record','module','plan-set','plan','plugin','profile','rule-execution-plan','stage-result','state')
