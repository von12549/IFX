...
$fixtures = [ordered]@{
    ...
    'workspace-evidence' = [ordered]@{ formatVersion=1; scope='v4-workspace-evidence-v1'; targetCommit=$commit; relativeRoots=@('src'); extensions=@('.cs'); excludedDirectoryNames=@('bin','obj'); pathOrder='ordinal'; fileCount=1; treeSha256=$hash; startedAt='2026-09-26T00:00:00.0000000+00:00'; completedAt='2026-09-26T00:00:00.1000000+00:00'; elapsedSeconds=0.1; files=@([ordered]@{path='src/App.cs';extension='.cs';length=12;sha256=$hash;normalizedSha256=$hash;text="class App {}`n"}) }
