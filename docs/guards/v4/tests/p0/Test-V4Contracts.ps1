...
$fixtures = [ordered]@{
    ...
    'distribution-manifest' = [ordered]@{ formatVersion=1; id='v4-guards'; version='0.1.0'; rootDirectory='v4-guards-0.1.0'; source=[ordered]@{ commit=$commit; packageHash=$hash; contractsManifestSha256=$hash; hostSha256=$hash }; files=@([ordered]@{ path='package/plugin.json'; kind='package'; sha256=$hash; size=1 },[ordered]@{ path='host/v4-guards.dll'; kind='host'; sha256=$hash; size=1 }) }
    'install-receipt' = [ordered]@{ formatVersion=1; id=('a' * 32); status='installed'; version='0.1.0'; installRoot='C:/v4/v4-guards-0.1.0'; archiveSha256=$hash; manifestSha256=$hash; files=@([ordered]@{ path='distribution-manifest.json'; sha256=$hash; size=1 },[ordered]@{ path='package/plugin.json'; sha256=$hash; size=1 }) }
    ...
    'compatibility-baseline' = [ordered]@{ formatVersion=1; pluginVersion='0.1.0'; apiVersion='1.0'; files=@(1..12|ForEach-Object{[ordered]@{path="core/contracts/fixture-$_.json";role=if($_-eq1){'cli'}elseif($_-eq2){'configuration'}else{'report'};sha256=$hash}}) }
