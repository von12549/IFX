...
try {
    ...
    $generatedProject = Join-Path $generatedRoot 'Sample.Guards.StageGate.Tests.csproj'
    ...
    $hookOutput = @(& pwsh -NoProfile -File (Join-Path $package 'hooks/Invoke-PlanHook.ps1') -ProfileDirectory $profile -TargetRoot $fixture -GenerationRoot $generationRoot -OutputDirectory $output -PlannedPaths 'src/App/Program.cs' 2>&1)
