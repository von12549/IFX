...
/// The command line and Model Context Protocol entry point of the engine. A host registers its policy bindings first and
/// then calls Run, so one contract serves the generic engine and a repository host alike (Plan 06 P6.3, D27).
public static class Cli
{
    ...
    public static int Run(string[] args)
    {
        ...
        // Tools live in the engine assembly, which is not the entry assembly of a repository host.
        builder.Services.AddMcpServer().WithStdioServerTransport().WithToolsFromAssembly(typeof(Tools).Assembly);
        builder.Build().Run();
        return 0;
    }
    ...
    private static string Require(string[] positional, string usage) =>
