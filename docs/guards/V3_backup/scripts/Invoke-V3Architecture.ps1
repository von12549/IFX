...
$architecturePath = Join-Path $analysis 'ARCHITECTURE.md'
$technicalPath = Join-Path $analysis 'TECHNICAL.md'
...
& (Join-Path $packageRoot 'scripts/Invoke-V3.ps1') -Mode Validate -TargetRoot $root -ProfileDirectory $proposalRoot -OutputDirectory $analysis | Out-Null
...
Write-Lf (Join-Path $destination 'README.md') "# Adopted architecture profile`n`nCreated from reviewed ARCHITECTURE.md and TECHNICAL.md in ``$([IO.Path]::GetRelativePath($root, $analysis).Replace('\', '/'))``. Run Validate, Generate, Check and detector tests before using this profile as a merge gate. Independent policies and CI require separate review."
