...
if (-not $OutputDirectory) { throw 'Analyze requires OutputDirectory.' }
...
Write-Lf (Join-Path $output 'PROPOSAL.md') "# Guard profile review proposal`n`nThis is a read-only analysis result, not an enabled policy. No rule, owner, command, negative fixture or CI required check is inferred as authoritative.`n`n## Candidate areas`n`n$($candidateLines -join "`n")`n`n## Review before enforcement`n`n1. Confirm source roots and owners against [inventory.json](inventory.json), repository guidance and maintainers.`n2. Choose real build/test commands and risk paths; compare existing architecture gate and CI authority.`n3. Write rule JSON with explicit detector scope and a deliberate negative fixture. Keep unsupported rules advisory.`n4. Run Validate, Render/Check, Generate/Check/Test and a violating fixture before enabling a required CI check."
