...
| CP07c-prep | P6.4（前置） | engine 的 V3 目标项目与测试路径（`Guards.ArchitectureConformance*`）、通用测试与 fixture 迁入 V3、IFX 专属 binding fixture、跨包 Check（D29，expand） | 有（移动） | 21 条 `move` 与 `change-trusted-base`：CP07c-prep-auth → CP07c-prep-change | CP07b | 已完成，待 PR 合入（`20260918-v3-stage-cp07c-prep-authorization` → `20260918-v3-stage-cp07c-prep-v3-engine-paths`） |
