...
| CP07b-prep | P6.3（前置） | 显式引用的 IFX facade 项目 `src/LayerGuard.Ifx` 与 base-owned `tests/LayerGuard.Ifx.Tests`，`GatePolicyBindingTests` 迁入；runner 检查精确项目集合与引用（D27，expand） | 有（移动） | `move` 与 `change-trusted-base`：CP07b-prep-auth → CP07b-prep-change | CP07a | 已完成，待 PR 合入（`20260918-v3-stage-cp07b-prep-authorization` → `20260918-v3-stage-cp07b-prep-ifx-facade`） |
