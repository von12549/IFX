...
| CP07b-prep | P6.3（前置） | 显式引用的 IFX facade 项目 `src/LayerGuard.Ifx` 与 base-owned `tests/LayerGuard.Ifx.Tests`，`GatePolicyBindingTests` 迁入；runner 检查精确项目集合与引用（D27，expand） | 有（移动） | `move` 与 `change-trusted-base`：CP07b-prep-auth → D22 撤销过期记录 → CP07b-prep-auth-r2 → CP07b-prep-change | CP07a | 已完成，待 PR 合入（`20260918-v3-stage-cp07b-prep-authorization` → `20260918-v3-stage-cp07b-prep-ifx-facade`） |
| CP07-ci | §10.3（配额） | CI 成本控制（concurrency 取消、package cache、月度 schedule）与 base 判定的变更范围：records-and-plans 时跳过 head candidate 步骤并让 architecture/quality/database 继承 base 判定 | 无 | `change-trusted-base` 与 `weaken-policy`：CP07-ci-auth → CP07-ci-change | CP07b-prep | 已完成，待 PR 合入（`20260918-v3-stage-cp07ci-authorization` → `20260918-v3-stage-cp07ci-cost-controls`） |
