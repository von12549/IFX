...
| CP06b2 | P4 | D18 domain authority 覆盖：base verifier 针对明确 PR head SHA 重新计算 `weaken-policy` 覆盖，报告绑定 base/merge-base/head 与 registry/schema hash；此前 D18 blocking findings 失败关闭 | 无 | `change-trusted-base` 与 `weaken-policy`：CP06b2-auth → CP06b2-change（验证 CP06b1 新规则生效） | CP06b1 | 已完成，待 PR 合入（`20260917-v3-stage-cp06b2-authorization` → `20260917-v3-stage-cp06b2-domain-authority-coverage`） |
