...
| CP08-prep | P7（前置） | IFX overlay、command manifest 与 base-owned validation 切换到 canonical V3；trusted-base 显式传递仓库外 generation root；legacy workflow runner 成为薄 wrapper（D30） | 无 | `change-trusted-base`：CP08-prep-auth-r2 → CP08-prep-change | CP08-prep0 | 本地 change 已构建，待可信验证 |
