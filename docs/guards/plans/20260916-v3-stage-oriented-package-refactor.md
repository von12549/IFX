...
| CP06c | P4 | 临时仓库两 PR 演练（`move`、`weaken-policy`、`change-trusted-base`）与证据、ruleset `strict` 断言证据、`Test-V3.ps1` parity 检查 | 无 | `change-trusted-base`：CP06c-auth → CP06c-change | CP06b2 | 进行中（`20260917-v3-stage-cp06c-authorization`、`20260917-v3-stage-cp06c-rehearsal-evidence`） |
