...
| CP12 | P11 | 新旧 parity、失败关闭场景、隔离 Bootstrap、真实 PR 激活、旧入口清理、回退演练与已合并子分支清理 | 有 | 清理删除：auth → change；激活需单独实施授权 | CP11 | 已完成：P11.1–P11.4 的最终候选、Windows full dispatch 与 activation 链全部通过；P11.5–P11.6 由 PR #104（`d980fdf7`，run `35583657297` 13/13）完成九个 wrapper 删除、授权消费与精确恢复演练；P11.7 删除并复核 51 个本地与 70 个远端已吸收分支，216 个未证明 ref 按契约保留。 |
