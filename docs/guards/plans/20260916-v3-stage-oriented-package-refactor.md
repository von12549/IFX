...
| CP11 | P10 | 按 Plan 06 §13 分组物理迁移、V3_backup 删除、兼容 wrapper | 有 | 每个迁移分组 auth → change | CP10 | 进行中：maintenance relocation 已完成；V3_backup retirement bridge `2e0e45ff` → `ff191f84`，final auth `7887eab1` → change `c12cc9a6`，41 个重复文件已删除且 43 项保护义务/TCB parity/隔离回归通过（2026-09-20，本地，未 push/PR） |
