...
| CP08 | P7 | Stage Gate 参数化命名、仓库外生成、取消跟踪 `generated/stages`、overlay contract 并删除重复实现（保留声明期内的薄 public wrapper） | 有 | `move`、`delete`、`change-trusted-base`、`weaken-policy`：CP08-auth-r2 → CP08-change | CP08-prep | exact candidate `2448cec0` 的十条 r2 授权已由独立 base 验证（`2cbf9e9d`）；待消费验证 |
