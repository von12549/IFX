...
| CP07-ci | §10.3（配额） | CI 成本控制（concurrency 取消、package cache、月度 schedule）与 base 判定的变更范围：records-and-plans 时跳过 head candidate 步骤并让 architecture/quality/database 继承 base 判定 | 无 | `change-trusted-base` 与 `weaken-policy`：CP07-ci-auth → CP07-ci-change | CP07b-prep | 进行中（`20260918-v3-stage-cp07ci-authorization`、`20260918-v3-stage-cp07ci-cost-controls`） |
