...
规则与权威入口逐项见 [生成规则索引](../generated/INDEX.md)和[覆盖矩阵](../generated/COVERAGE_MATRIX.md)。LayerGuard 的 `notChecked` 明确包含“semantic symbol binding”；本次未把语法匹配升级宣称为语义证明。`ARCH.SEMANTIC` 保持 `none/advisory`，由架构维护者审查，待编译器语义检测器有真实正反 fixture 后再考虑 blocking。既有行为、契约、迁移验证仍以各专用 Gate 报告为证据，不由新规则名代替。

| 规则 ID | 关联的实际验证入口 | 仍未证明的部分 |
| --- | --- | --- |
| `L1.2`、`L2.*`、`L3.*` | `Invoke-LayerGuard.ps1` 及其 190 项工具测试 | 语义绑定、实际运行时调用与完整业务行为 |
...
仓库文件已声明新 `Coding Guardrails` 的 `guard-framework`、`framework-windows`、`frontend-quality` 三个 job；PR 上运行再生成 Check、框架测试、显式 base/head SHA 的 Diff，main push 运行再生成 Check 与框架测试。Linux 和 Windows CI 尚未实际运行，因为本分支尚未推送/创建 PR。新 workflow 不重复执行旧 Gate；现有 workflow 状态仍需独立审查。

读取 GitHub `main` branch protection 与 rulesets 的 API 均返回 HTTP 403：`Upgrade to GitHub Pro or make this repository public to enable this feature.` 因此目前**没有证据**表明新旧 checks 已被设为 required，也无法在当前仓库套餐下完成这项外部配置。不能把 workflow 文件存在等同于合入门禁生效。仓库所有者需要在该功能可用后，把新三个 job 和既有必需 Gate job 加入 required checks，并用一次故意不再生成的 PR 证明阻断；之后再更新本记录。
