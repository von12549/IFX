...
Local completion chain: bridge authorization `2e0e45ff` → consumed bridge `ff191f84`; physical-retirement authorization `7887eab1` → consumed change `c12cc9a6`. The protected-change report covers 43 obligations with three consumed authorizations. Base-owned TCB validation reports equal verdicts for Validate, HistoricalIntegrity, G03, G04, G05 and Plan04. Full target-root separation and the IFX package suite pass. Evidence is under `artifacts/guards/v3-ifx/cp11b-v3-backup-retirement/`.
