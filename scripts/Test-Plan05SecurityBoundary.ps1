...
$resource = Read-Source 'src/Modules/IAM/IFX.Modules.IAM.Application/Access/ResourceAuthorizationService.cs'
...
$checks.internalAuthorizationRequiresExplicitContext = $permission -match '_execution.HasCurrent' -and $permission -match 'ActorKind.User' -and $permission -match '_currentUser.TenantId == _execution.Current.TenantId' -and $resource -match 'execution.HasCurrent' -and $resource -match 'ActorKind.User' -and $resource -match 'contract_context_invalid'
