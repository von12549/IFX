...
$diagramDirectory = Join-Path $repositoryRoot 'docs/architecture/review/gates/G05/diagrams'
$sources = @(Get-ChildItem -LiteralPath $diagramDirectory -Filter '*.mmd' | Sort-Object Name)
if ($sources.Count -ne 6) { throw "Expected exactly six G05 Mermaid sources; found $($sources.Count)." }
...
Write-Host "Rendered $($sources.Count) G05 diagrams to SVG and PNG with Mermaid CLI $MermaidCliVersion."
