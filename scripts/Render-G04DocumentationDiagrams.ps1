[CmdletBinding()]
param([string] $MermaidCliVersion = '11.17.0')

$ErrorActionPreference = 'Stop'
$repositoryRoot = Split-Path -Parent $PSScriptRoot
$diagramDirectory = Join-Path $repositoryRoot 'docs/architecture/review/gates/G04/diagrams'
$sources = Get-ChildItem -LiteralPath $diagramDirectory -Filter '*.mmd' | Sort-Object Name
if ($sources.Count -ne 6) { throw "Expected exactly six G04 Mermaid sources; found $($sources.Count)." }
...
Write-Host "Rendered $($sources.Count) G04 diagrams to SVG and PNG with Mermaid CLI $MermaidCliVersion."
