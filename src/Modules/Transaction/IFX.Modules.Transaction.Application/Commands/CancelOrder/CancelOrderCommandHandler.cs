    ...
    public async Task<Result<OrderDto>> Handle(CancelOrderCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            await _authorizationService.AuthorizeWithResolvedPolicyAsync(
                "order", "delete", new TenantScopeResourceAttributes(_currentUser.TenantId), ct: cancellationToken);
        ...
        catch (Exception ex)
        {
            ...
            return Result<OrderDto>.Failure("An error occurred while cancelling the order.");
        }
    }
}
