...
public class CreateOrderCommandHandler : IRequestHandler<CreateOrderCommand, Result<OrderDto>>
{
    ...
    public CreateOrderCommandHandler(
        ...
        IIntegrationEventBus eventBus, ILogger<CreateOrderCommandHandler> logger)
    {
    ...
    public async Task<Result<OrderDto>> Handle(CreateOrderCommand request, CancellationToken cancellationToken)
    {
        try
        {
            await _authorizationService.AuthorizeWithResolvedPolicyAsync(
                "order", "create", new TenantScopeResourceAttributes(_currentUser.TenantId), ct: cancellationToken);
            ...
            if (orderType == "SUBSCRIPTIONORDER")
            {
                ...
                order = Order.CreateSubscriptionOrder(
                    tenantId, request.OrderReference,
                    request.InvestmentAccountId, request.FromFundId, request.FromClassId,
                    request.Amount, request.Currency, request.TradeDate);
            }
            else if (orderType == "REDEMPTIONORDER")
            {
                order = Order.CreateRedemptionOrder(
                    tenantId, request.OrderReference,
                    request.InvestmentAccountId, request.FromFundId, request.FromClassId,
                    request.Amount, request.Currency, request.TradeDate);
            }
            else // SwitchOrder
            {
                ...
                order = Order.CreateSwitchOrder(
                    tenantId, request.OrderReference,
                    request.InvestmentAccountId,
                    request.FromFundId, request.FromClassId,
                    request.ToFundId.Value, request.ToClassId.Value,
                    request.Amount, request.Currency, request.TradeDate);
            }
            ...
            await _eventBus.PublishAsync(
                new OrderSubmittedEvent(order.Id, tenantId, order.OrderType.ToString(), order.OrderReference, order.Legs.Count),
                cancellationToken);
