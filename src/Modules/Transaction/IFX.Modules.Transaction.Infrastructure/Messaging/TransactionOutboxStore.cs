...
public sealed class TransactionOutboxStore(TransactionDbContext dbContext) : IModuleOutboxStore
{
    ...
    public async Task<IReadOnlyList<OutboxDispatchLease>> ClaimAsync(OutboxClaimRequest request, CancellationToken cancellationToken)
    {
        ...
        var candidates = await dbContext.OutboxMessages.FromSqlInterpolated($$"""
            ...
            FROM [transaction].[OutboxMessages] AS [candidate] WITH (UPDLOCK, READPAST, ROWLOCK)
            WHERE [candidate].[State] = N'Pending'
              ...
              AND NOT EXISTS (
                  SELECT 1 FROM [transaction].[OutboxMessages] AS [earlier]
    ...
    internal static TransactionOutboxMessage ToEntity(OutboxLogicalMessage logical, string partitionKey, long sequence) => new()
    {
    ...
    private static OutboxDispatchLease ToLease(TransactionOutboxMessage message) => new(ToLogical(message), "transaction", message.PartitionKey, message.Sequence, message.LeaseOwner!, message.LeaseUntil!.Value, message.ConcurrencyToken, message.AttemptCount);

    private static OutboxLogicalMessage ToLogical(TransactionOutboxMessage message) => new(
    ...
    private async Task<TransactionOutboxMessage> OwnedLeaseAsync(OutboxDispatchLease lease, CancellationToken cancellationToken)
    {
        ...
        if (!message.ConcurrencyToken.SequenceEqual(lease.ConcurrencyToken)) throw new DbUpdateConcurrencyException("The Transaction Outbox lease is stale.");
        return message;
    }
}
