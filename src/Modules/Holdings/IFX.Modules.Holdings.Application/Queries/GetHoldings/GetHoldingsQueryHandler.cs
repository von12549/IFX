...
public class GetHoldingsQueryHandler : IRequestHandler<GetHoldingsQuery, Result<IReadOnlyList<HoldingSummaryDto>>>
{
    ...
    public GetHoldingsQueryHandler(IUnitOfWork unitOfWork, IMapper mapper, ICurrentUser currentUser,
        IResourceAuthorizationService authorizationService, ILogger<GetHoldingsQueryHandler> logger)
    {
        _unitOfWork = unitOfWork; _mapper = mapper; _currentUser = currentUser;
        _authorizationService = authorizationService; _logger = logger;
    }

    public async Task<Result<IReadOnlyList<HoldingSummaryDto>>> Handle(GetHoldingsQuery request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var holdings = await _unitOfWork.Holdings.GetByTenantAsync(_currentUser.TenantId.Value, cancellationToken);
            var dtos = _mapper.Map<IReadOnlyList<HoldingSummaryDto>>(holdings);
            return Result<IReadOnlyList<HoldingSummaryDto>>.Success(dtos);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error listing holdings");
