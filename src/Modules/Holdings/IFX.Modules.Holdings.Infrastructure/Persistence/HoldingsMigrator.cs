...
public sealed class HoldingsMigrator : IAppMigrator
{
    ...
    private const string InitialCreateId = "20260413120100_InitialCreate";
    ...
    private async Task StampIfEnsureCreatedDatabaseAsync(HoldingsDbContext db, CancellationToken ct)
    {
        ...
        var exists = (await db.Database.SqlQueryRaw<int>(
            ...
            "WHERE TABLE_SCHEMA = 'holdings' AND TABLE_NAME = 'Holdings'").ToListAsync(ct)).FirstOrDefault() > 0;
        if (!exists) return;
