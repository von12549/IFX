...
public class LinkAdvisorToInvestmentAccountCommandHandler : IRequestHandler<LinkAdvisorToInvestmentAccountCommand, Result<Unit>>
{
    ...
    public LinkAdvisorToInvestmentAccountCommandHandler(IUnitOfWork unitOfWork, ICurrentUser currentUser,
        IResourceAuthorizationService authorizationService, ILogger<LinkAdvisorToInvestmentAccountCommandHandler> logger)
    {
    ...
    public async Task<Result<Unit>> Handle(LinkAdvisorToInvestmentAccountCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var hasAdvisoryRole =
                await _unitOfWork.PartyRoleAssignments.HasRoleAsync(request.AdvisorPartyId, PartyFunctionalRole.AdvisoryFirm, tenantId, cancellationToken)
                || await _unitOfWork.PartyRoleAssignments.HasRoleAsync(request.AdvisorPartyId, PartyFunctionalRole.AdvisoryBranch, tenantId, cancellationToken)
                || await _unitOfWork.PartyRoleAssignments.HasRoleAsync(request.AdvisorPartyId, PartyFunctionalRole.AdvisorRep, tenantId, cancellationToken);
            if (!hasAdvisoryRole)
            ...
            var link = AdvisorInvestmentAccountLink.Create(tenantId, request.AdvisorPartyId, request.InvestmentAccountId,
                request.EffectiveDate, request.RebateRate);
        ...
        {
            _logger.LogError(ex, "Error linking advisor to account");
