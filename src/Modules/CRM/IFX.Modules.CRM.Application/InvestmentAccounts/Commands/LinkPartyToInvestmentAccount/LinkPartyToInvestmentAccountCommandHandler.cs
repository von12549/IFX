...
public class LinkPartyToInvestmentAccountCommandHandler : IRequestHandler<LinkPartyToInvestmentAccountCommand, Result<Unit>>
{
    ...
    public LinkPartyToInvestmentAccountCommandHandler(IUnitOfWork unitOfWork, ICurrentUser currentUser, IResourceAuthorizationService authorizationService, ILogger<LinkPartyToInvestmentAccountCommandHandler> logger)
    {
    ...
    {
        {
            ...
            if (request.RelationshipType == InvestmentAccountRelationshipType.RegisteredHolder && account.AccountType != InvestmentAccountType.Joint && existingLinks.Any(l => l.RelationshipType == InvestmentAccountRelationshipType.RegisteredHolder))
            ...
            {
                var existingTotal = existingLinks.Where(l => l.RelationshipType == InvestmentAccountRelationshipType.RegisteredHolder).Sum(l => l.OwnershipPercentage ?? 0m);
            ...
            var link = PartyInvestmentAccountLink.Create(tenantId, request.PartyId, request.InvestmentAccountId, request.RelationshipType, request.EffectiveDate, request.OwnershipPercentage, request.LinkOrder);
