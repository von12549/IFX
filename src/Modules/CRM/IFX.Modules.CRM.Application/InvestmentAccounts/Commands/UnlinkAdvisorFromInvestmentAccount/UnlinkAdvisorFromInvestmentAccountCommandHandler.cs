...
public class UnlinkAdvisorFromInvestmentAccountCommandHandler : IRequestHandler<UnlinkAdvisorFromInvestmentAccountCommand, Result<Unit>>
{
    ...
    public UnlinkAdvisorFromInvestmentAccountCommandHandler(IUnitOfWork unitOfWork, ICurrentUser currentUser,
        IResourceAuthorizationService authorizationService, ILogger<UnlinkAdvisorFromInvestmentAccountCommandHandler> logger)
    {
    ...
    public async Task<Result<Unit>> Handle(UnlinkAdvisorFromInvestmentAccountCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (link == null) return Result<Unit>.Failure("Advisor link not found.");
        ...
        {
            _logger.LogError(ex, "Error unlinking advisor from account");
