...
public class UnlinkPartyFromInvestmentAccountCommandHandler : IRequestHandler<UnlinkPartyFromInvestmentAccountCommand, Result<Unit>>
{
    ...
    public UnlinkPartyFromInvestmentAccountCommandHandler(IUnitOfWork unitOfWork, ICurrentUser currentUser,
        IResourceAuthorizationService authorizationService, ILogger<UnlinkPartyFromInvestmentAccountCommandHandler> logger)
    {
    ...
    public async Task<Result<Unit>> Handle(UnlinkPartyFromInvestmentAccountCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (link == null) return Result<Unit>.Failure("Party link not found.");
        ...
        {
            _logger.LogError(ex, "Error unlinking party");
