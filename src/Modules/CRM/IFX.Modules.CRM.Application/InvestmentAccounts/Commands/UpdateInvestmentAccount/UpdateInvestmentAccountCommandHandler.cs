...
public class UpdateInvestmentAccountCommandHandler : IRequestHandler<UpdateInvestmentAccountCommand, Result<InvestmentAccountDto>>
{
    ...
    public UpdateInvestmentAccountCommandHandler(IUnitOfWork unitOfWork, IMapper mapper,
        ICurrentUser currentUser, IResourceAuthorizationService authorizationService,
        ILogger<UpdateInvestmentAccountCommandHandler> logger)
    {
    ...
    public async Task<Result<InvestmentAccountDto>> Handle(UpdateInvestmentAccountCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogError(ex, "Error updating investment account {Id}", request.Id);
