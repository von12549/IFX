...
public class DeleteInvestmentAccountCommandHandler : IRequestHandler<DeleteInvestmentAccountCommand, Result<Unit>>
{
    ...
    public DeleteInvestmentAccountCommandHandler(IUnitOfWork unitOfWork, ICurrentUser currentUser,
        IResourceAuthorizationService authorizationService, ILogger<DeleteInvestmentAccountCommandHandler> logger)
    {
    ...
    public async Task<Result<Unit>> Handle(DeleteInvestmentAccountCommand request, CancellationToken cancellationToken)
    {
        try
        {
            await _authorizationService.AuthorizeWithResolvedPolicyAsync(
                "investment-account", "delete", new TenantScopeResourceAttributes(_currentUser.TenantId), ct: cancellationToken);
        ...
        {
            _logger.LogError(ex, "Error deleting investment account {Id}", request.Id);
