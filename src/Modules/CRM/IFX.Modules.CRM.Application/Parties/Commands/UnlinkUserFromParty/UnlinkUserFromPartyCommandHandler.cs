...
public class UnlinkUserFromPartyCommandHandler : IRequestHandler<UnlinkUserFromPartyCommand, Result<Unit>>
{
    ...
    public UnlinkUserFromPartyCommandHandler(IUnitOfWork unitOfWork, ICurrentUser currentUser,
        IResourceAuthorizationService authorizationService, ILogger<UnlinkUserFromPartyCommandHandler> logger)
    {
    ...
    public async Task<Result<Unit>> Handle(UnlinkUserFromPartyCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (link == null) return Result<Unit>.Failure("User party link not found.");
        ...
        {
            _logger.LogError(ex, "Error unlinking user from party");
