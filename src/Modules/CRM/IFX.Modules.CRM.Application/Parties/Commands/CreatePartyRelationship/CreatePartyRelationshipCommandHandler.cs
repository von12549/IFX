...
public class CreatePartyRelationshipCommandHandler : IRequestHandler<CreatePartyRelationshipCommand, Result<Unit>>
{
    ...
    public async Task<Result<Unit>> Handle(CreatePartyRelationshipCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (directionError != null) return Result<Unit>.Failure(directionError);

            var existing = await _unitOfWork.PartyRelationships.GetAsync(
                request.FromPartyId, request.ToPartyId, request.RelationshipType, tenantId, cancellationToken);
            ...
            var relationship = PartyRelationship.Create(tenantId, request.FromPartyId, request.ToPartyId,
                request.RelationshipType, request.EffectiveDate);
        ...
        {
            _logger.LogError(ex, "Error creating party relationship");
    ...
    private async Task<string?> ValidateDirectionAsync(CreatePartyRelationshipCommand request, Guid tenantId, CancellationToken ct)
    {
        switch (request.RelationshipType)
        {
            case PartyRelationshipType.ParentFirm:
                var fromHasAdvisoryRole =
                    await _unitOfWork.PartyRoleAssignments.HasRoleAsync(request.FromPartyId, PartyFunctionalRole.AdvisorRep, tenantId, ct)
                    || await _unitOfWork.PartyRoleAssignments.HasRoleAsync(request.FromPartyId, PartyFunctionalRole.AdvisoryBranch, tenantId, ct);
                if (!fromHasAdvisoryRole)
                    return "For ParentFirm: FromParty must have role AdvisorRep or AdvisoryBranch.";
                var toHasFirmRole =
                    await _unitOfWork.PartyRoleAssignments.HasRoleAsync(request.ToPartyId, PartyFunctionalRole.AdvisoryFirm, tenantId, ct)
                    || await _unitOfWork.PartyRoleAssignments.HasRoleAsync(request.ToPartyId, PartyFunctionalRole.AdvisoryBranch, tenantId, ct);
                if (!toHasFirmRole)
                    return "For ParentFirm: ToParty must have role AdvisoryFirm or AdvisoryBranch.";
                break;

            case PartyRelationshipType.AuthorizedToAdvise:
                var fromHasAnyAdvisoryRole =
                    await _unitOfWork.PartyRoleAssignments.HasRoleAsync(request.FromPartyId, PartyFunctionalRole.AdvisorRep, tenantId, ct)
                    || await _unitOfWork.PartyRoleAssignments.HasRoleAsync(request.FromPartyId, PartyFunctionalRole.AdvisoryFirm, tenantId, ct)
                    || await _unitOfWork.PartyRoleAssignments.HasRoleAsync(request.FromPartyId, PartyFunctionalRole.AdvisoryBranch, tenantId, ct);
