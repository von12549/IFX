...
public class CreatePartyCommandHandler : IRequestHandler<CreatePartyCommand, Result<PartyDto>>
{
    ...
    public CreatePartyCommandHandler(IUnitOfWork unitOfWork, IMapper mapper, ICurrentUser currentUser, IResourceAuthorizationService authorizationService, ICommittedEventBuffer eventBuffer, ILogger<CreatePartyCommandHandler> logger)
    {
    ...
    {
        {
            await _authorizationService.AuthorizeWithResolvedPolicyAsync("party", "create", new TenantScopeResourceAttributes(_currentUser.TenantId), ct: cancellationToken);
