...
public class AddInvestorDocumentCommandHandler : IRequestHandler<AddInvestorDocumentCommand, Result<Unit>>
{
    ...
    public AddInvestorDocumentCommandHandler(IUnitOfWork unitOfWork, ICurrentUser currentUser, IResourceAuthorizationService authorizationService, ILogger<AddInvestorDocumentCommandHandler> logger)
    {
    ...
    {
        {
            ...
            if (investor == null)
                return Result<Unit>.Failure("Investor not found.");
            var doc = InvestorDocument.Create(request.InvestorId, tenantId, request.DocumentType, request.DocumentNumber, request.IssueCountry, request.IssueState, request.IssueDate, request.ExpiryDate);
