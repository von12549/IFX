...
public class UpdateProductCommandHandler : IRequestHandler<UpdateProductCommand, Result<ProductDto>>
{
    ...
    public UpdateProductCommandHandler(IUnitOfWork unitOfWork, IMapper mapper, ICurrentUser currentUser, IResourceAuthorizationService authorizationService, ILogger<UpdateProductCommandHandler> logger)
    {
    ...
    {
        {
            ...
            await _authorizationService.AuthorizeWithResolvedPolicyAsync("product", "update", new TenantScopeResourceAttributes(tenantId), ct: cancellationToken);
            var product = await _unitOfWork.Products.GetByIdAsync(request.ProductId, tenantId.Value, cancellationToken);
            if (product == null)
                return Result<ProductDto>.Failure("Product not found.");
            product.Update(request.ProductName, request.ApirCode, request.Isin, request.RegulatorSchemeNumber, request.PdsReference, request.IssuerName, request.WindUpDate);
