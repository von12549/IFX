...
public class CreateClassCommandHandler : IRequestHandler<CreateClassCommand, Result<FundClassDto>>
{
    ...
    public async Task<Result<FundClassDto>> Handle(CreateClassCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var fundClass = FundClass.Create(
                request.FundId,
                tenantId.Value,
                request.ClassCode,
                request.ClassName,
            ...
            if (request.MinInitialInvestment.HasValue ||
                request.ManagementFeeRate.HasValue ||
                request.PerformanceFeeRate.HasValue)
            {
                fundClass.Update(
                    request.ClassName,
                    request.Currency,
            ...
            await _eventBus.PublishAsync(new ClassCreatedEvent(fundClass.Id, fundClass.FundId, fundClass.TenantId, fundClass.ClassCode), cancellationToken);
        ...
        {
            _logger.LogError(ex, "Error creating fund class {ClassCode}", request.ClassCode);
            return Result<FundClassDto>.Failure("An error occurred while creating the fund class.");
        }
    }
}
