    ...
    {
        {
            ...
            var cognitoResult = await _identityProvider.SignUpAsync(request.Email, request.Password, request.Username, request.FirstName, request.LastName, request.BirthDate, request.PhoneNumber);
            if (!cognitoResult.Success)
            {
                return Result<RegisterUserResponse>.Failure(cognitoResult.ErrorMessage ?? "Failed to create user in Cognito");
            }
            ...
            var userIdentity = UserIdentity.Create(userId: user.Id, idpId: primaryIdp.Id, issuer: primaryIdp.Issuer, subject: Subject.Create(cognitoResult.Subject!), email: EmailAddress.Create(request.Email), firstName: request.FirstName, lastName: request.LastName, birthDate: request.BirthDate, phoneNumber: request.PhoneNumber, emailVerified: cognitoResult.UserConfirmed, phoneNumberVerified: false);
            ...
            var registrationEvent = RegistrationFlowEvent.Create(request.Email, request.Username, request.IpAddress);
            await _unitOfWork.RegistrationFlowEvents.AddAsync(registrationEvent, cancellationToken);
            // Create UserActivityLog
            var activityLog = UserActivityLog.Create(user.Id, ActivityType.Registration, $"User {request.Username} registered", request.IpAddress);
            await _unitOfWork.UserActivityLogs.AddAsync(activityLog, cancellationToken);
            _logger.LogInformation("User {Username} registered successfully with Subject {Subject}", request.Username, cognitoResult.Subject);
            return Result<RegisterUserResponse>.Success(new RegisterUserResponse { UserId = user.Id, Subject = cognitoResult.Subject!, RequiresConfirmation = !cognitoResult.UserConfirmed, Message = cognitoResult.UserConfirmed ? "Registration successful" : "Registration successful. Please check your email for confirmation code." });
        }
    }
}
