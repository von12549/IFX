    ...
    public async Task<Result<PermissionDto>> Handle(UpdatePermissionCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (permission.Name != request.Name &&
                await _unitOfWork.Permissions.NameExistsAsync(request.Name, request.PermissionId, cancellationToken))
        ...
        {
            _logger.LogError(ex, "Error updating permission {PermissionId}", request.PermissionId);
            return Result<PermissionDto>.Failure("An error occurred while updating the permission");
        }
    }
}
